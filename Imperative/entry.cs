...
method encodePixelAEI(curr: RGBA, prev: RGBA, index: array<RGBA>, ghost state: State, encoding: array<Op>, len: int)
    returns (newlen: int)
  ...
  requires state == updateStateStar(initState(), specOps(encoding[..len]))
  modifies index, encoding
  ...
  decreases curr, prev, index, state, encoding, len
{
  newlen := len + 1;
  if curr == prev {
    if len > 0 && encoding[len - 1].OpRun? && encoding[len - 1].size < 62 {
      ...
      assert pixels1 == seq(old(encoding[len - 1].size), (i: int) => state_prev.prev);
      assert pixels2 == seq(old(encoding[len - 1].size) + 1, (i: int) => state_prev.prev);
  ...
  decreases image
{
  ...
  while i < wh
    ...
    invariant state == updateStateStar(initState(), image[..i])
...
ghost predicate updatedState(state: State, newstate: State, pixels: seq<RGBA>)
  requires validState(state)
  decreases state, newstate, pixels
{
  newstate == updateStateStar(state, pixels)
}

lemma updatedStateNext(state: State, oldnewstate: State, newstate: State, pixels: seq<RGBA>, pixel: RGBA)
  requires validState(state)
  ...
  decreases state, oldnewstate, newstate, pixels, pixel
{
}

method decodeOp(state: State, op: Op, len: int, res: array<RGBA>)
    returns (newlen: int, newstate: State, ok: bool)
  requires validState(state)
  requires 0 <= len
  modifies res
  ...
  decreases state, op, len, res
{
  match op {
    case {:split false} OpRGB(RGB(r, g, b)) =>
      ok := len + 1 <= res.Length;
      if ok {
    ...
    case {:split false} OpRun(size) =>
      {
        var pixel := state.prev;
        var i: int := 0;
        newlen := len;
        newstate := state;
        var pixels := seq(size, (j: int) => pixel);
    ...
    case {:split false} OpIndex(index) =>
      ok := len + 1 <= res.Length;
      if ok {
    ...
    case {:split false} OpDiff(RGBDiff(dr, dg, db)) =>
      ok := len + 1 <= res.Length;
      if ok {
        var pixel := RGBA(add_byte(state.prev.r, byte_from(dr as int)), add_byte(state.prev.g, byte_from(dg as int)), add_byte(state.prev.b, byte_from(db as int)), state.prev.a);
        res[len] := pixel;
        newlen := len + 1;
        newstate := updateState(state, pixel);
      }
    case {:split false} OpLuma(RGBLuma(dr, dg, db)) =>
      ok := len + 1 <= res.Length;
      if ok {
        var pixel := RGBA(add_byte(add_byte(state.prev.r, byte_from(dg as int)), byte_from(dr as int)), add_byte(state.prev.g, byte_from(dg as int)), add_byte(add_byte(state.prev.b, byte_from(dg as int)), byte_from(db as int)), state.prev.a);
        res[len] := pixel;
        newlen := len + 1;
        newstate := updateState(state, pixel);
      }
...
lemma /*{:_inductionTrigger |data|}*/ /*{:_induction data}*/ asRGBA3Last(data: seq<byte>)
  requires |data| % 3 == 0
  requires |data| > 0
  ...
  decreases data
{
  if |data| == 3 {
    ...
    asRGBA3Last(data[3..]);
    assert asRGBA3(data[3..]) == asRGBA3(data[3..][..|data[3..]| - 3]) + [RGBA(data[3..][|data[3..]| - 3], data[3..][|data[3..]| - 2], data[3..][|data[3..]| - 1], 255)];
    ...
    ==
      [RGBA(data[0], data[1], data[2], 255)] + asRGBA3(data[3..]);
    ==
      [RGBA(data[0], data[1], data[2], 255)] + asRGBA3(data[3 .. |data| - 3]) + [RGBA(data[|data| - 3], data[|data| - 2], data[|data| - 1], 255)];
    ==
      [RGBA(data[0], data[1], data[2], 255)] + asRGBA3(data[3 .. |data| - 3]) + [RGBA(data[|data| - 3], data[|data| - 2], data[|data| - 1], 255)];
    ==
...
lemma /*{:_inductionTrigger |data|}*/ /*{:_induction data}*/ asRGBA4Last(data: seq<byte>)
  requires |data| % 4 == 0
  requires |data| > 0
  ...
  decreases data
{
  if |data| == 4 {
    ...
    asRGBA4Last(data[4..]);
    assert asRGBA4(data[4..]) == asRGBA4(data[4..][..|data[4..]| - 4]) + [RGBA(data[4..][|data[4..]| - 4], data[4..][|data[4..]| - 3], data[4..][|data[4..]| - 2], data[4..][|data[4..]| - 1])];
    ...
    ==
      [RGBA(data[0], data[1], data[2], data[3])] + asRGBA4(data[4 .. |data| - 4]) + [RGBA(data[|data| - 4], data[|data| - 3], data[|data| - 2], data[|data| - 1])];
    ==
      [RGBA(data[0], data[1], data[2], data[3])] + asRGBA4(data[4 .. |data| - 4]) + [RGBA(data[|data| - 4], data[|data| - 3], data[|data| - 2], data[|data| - 1])];
    ==
...
method writeBitsOp(op: Op, result: array<byte>, pos: int)
    returns (newpos: int)
  ...
  decreases op, result, pos
{
  ...
  match op {
    ...
    case {:split false} OpDiff(diff) =>
      result[pos] := 64 + 16 * encodeDiff(diff.dr) + 4 * encodeDiff(diff.dg) + encodeDiff(diff.db);
...
method encodeBitsOps(ops: array<Op>, opsLen: int, result: array<byte>, pos: int)
    returns (newpos: int)
  requires 0 <= opsLen <= ops.Length
...
lemma resultOk(image: Image, result: array<byte>, len: int, ops: array<Op>, opsLen: int)
  ...
  decreases image, result, len, ops, opsLen
{
  ...
  assert |specOps(specBits(byteStream[14 .. len - 8]))| == image.desc.width as int * image.desc.height as int;
  ...
  decreases byteStream
{
  ...
  if 3 <= byteStream[12] <= 4 && validColorSpaceAsByte(byteStream[13]) {
    var desc := Desc(pack(byteStream[4 .. 8]), pack(byteStream[8 .. 12]), byteStream[12] as Channels, colorSpaceFromByte(byteStream[13]));
...
method areValidBits(bits: array<byte>, start: int, len: int)
    returns (b: bool)
  requires 0 <= start <= start + len <= bits.Length
  ensures b == validBits(bits[start .. start + len])
  decreases bits, start, len
{
  b := len > 0 && match opTypeOfBits(bits[start + 0]) { case TypeRun() => len == 1 && bits[start + 0] >= 128 + 64 && bits[start + 0] - 128 - 64 <= 61 case TypeLuma() => len == 2 && bits[start + 0] >= 128 && bits[start + 0] < 128 + 64 case TypeDiff() => len == 1 && bits[start + 0] >= 64 && bits[start + 0] < 128 case TypeIndex() => len == 1 && bits[start + 0] < 64 case TypeRGBA() => len == 5 && bits[start + 0] == 255 case TypeRGB() => len == 4 && bits[start + 0] == 254 };
}
...
lemma /*{:_inductionTrigger encodeOpsSpec(ops)}*/ /*{:_induction ops}*/ encode_decode_ops(ops: seq<Op>)
...
lemma /*{:_inductionTrigger last(ops)}*/ /*{:_inductionTrigger init(ops)}*/ /*{:_inductionTrigger |ops|}*/ /*{:_induction ops}*/ encodeOpsSpecLast(ops: seq<Op>)
  requires |ops| > 0
  ...
  decreases ops
{
  if |ops| == 1 {
    ...
    ==
      encodeOpSpec(head(ops)) + encodeOpsSpec(tail(ops));
    ==
      encodeOpSpec(head(ops)) + encodeOpsSpec(init(tail(ops))) + encodeOpSpec(last(tail(ops)));
    ==
      {
    ...
    ==
      ...
      encodeOpSpec(head(init(ops))) + encodeOpsSpec(init(tail(ops))) + encodeOpSpec(last(tail(ops)));
    ==
      {
        init_tail_comm(ops);
        last_tail(ops);
      }
      encodeOpSpec(head(init(ops))) + encodeOpsSpec(tail(init(ops))) + encodeOpSpec(last(ops));
    ==
      encodeOpSpec(head(init(ops))) + encodeOpsSpec(tail(init(ops))) + encodeOpSpec(last(ops));
    ==
      {
      ...
      encodeOpsSpec(init(ops)) + encodeOpSpec(last(ops));
    }
  ...
  decreases bits
{
  if |bits| == 0 then
    ...
    var len: int := sizeBitEncoding(opTypeOfBits(bits[0])); [decodeBits(bits[0 .. len])] + specBits(bits[len..])
}

lemma /*{:_inductionTrigger bits[..]}*/ /*{:_inductionTrigger bits[..i]}*/ /*{:_inductionTrigger bits[i..]}*/ /*{:_inductionTrigger |bits|}*/ /*{:_induction bits}*/ helperDecode2(bits: seq<byte>, i: int)
  requires 0 <= i <= |bits|
  ...
  decreases bits, i
{
  if |bits[..i]| == 0 {
    ...
    ghost var len := sizeBitEncoding(opTypeOfBits(bits[0]));
    assert |bits[..i]| >= len;
    assert len <= i;
...
lemma helperSegment(bits: array<byte>, start: int, i: int, len: int)
  ...
  decreases bits, start, i, len
{
}

method decodeBitsOps(bits: array<byte>, start: int, end: int)
    returns (r: Option<array<Op>>, lenr: int)
  requires 0 <= start <= end <= bits.Length
  ensures !validBitSeq(bits[start .. end]) ==> r.None?
  ensures r.Some? ==> 0 <= lenr <= r.some.Length
  ensures validBitSeq(bits[start .. end]) ==> r.Some? && r.some[..lenr] == specBits(bits[start .. end])
  decreases bits, start, end
{
  var i := 0;
  var j := 0;
  ...
  decreases byteStream
{
  ...
  |specOps(specBits(byteStream[14 .. len - 8]))| == specHeaderDesc(byteStream[0 .. 14]).width as int * specHeaderDesc(byteStream[0 .. 14]).height as int
}
...
function specEndToEnd(byteStream: seq<byte>): Image
  ...
  decreases byteStream
{
  var desc: Desc := specHeaderDesc(byteStream[..14]);
  var len: int := |byteStream|;
...
lemma /*{:_inductionTrigger ops[..i], validState(state)}*/ /*{:_induction ops, state}*/ specOpsAuxMonotone(ops: seq<Op>, state: State, i: int)
  requires 0 <= i <= |ops|
  requires validState(state)
  ensures |specOpsAux(ops, state)| >= |specOpsAux(ops[..i], state)|
  decreases ops, state, i
{
  if |ops| == 0 {
  } else {
    if i == 0 {
    } else {
      ghost var pixels: seq<RGBA> := specDecodeOp(state, ops[0]);
      ghost var state' := updateStateStar(state, pixels);
...
lemma specOpsDecompose(ops: seq<Op>, i: int, state: State)
  ...
  decreases ops, i, state
{
  ghost var ops' := ops[..i + 1];
  assert ops'[..i] == ops[..i];
  assert ops'[i] == ops[i];
  specOpsAuxAssoc(ops', initState());
  assert specOpsAux(ops'[..|ops'| - 1], initState()) + specDecodeOp(updateStateStar(initState(), specOpsAux(ops'[..|ops'| - 1], initState())), ops'[|ops'| - 1]) == specOpsAux(ops', initState());
}
...
namespace _module {
  ...
  public partial class __default {
      ...
      if ((new BigInteger((_0_input).Length)) < (new BigInteger(8))) {
        ...
        if (_1_b) {
          ...
          if (((new BigInteger((_0_input).Length)) - (new BigInteger(8))) != (((new BigInteger(_2_width)) * (new BigInteger(_3_height))) * (new BigInteger(_4_channels)))) {
            ...
            _6_result = _out0;
            _7_len = _out1;
            BigInteger _8_repeat;
            _8_repeat = BigInteger.Zero;
            while ((_8_repeat) < (new BigInteger(99))) {
              Dafny.Helpers.Print((_8_repeat));
              _0_input = FileInput.Reader.getContent();
              _5_image = _module.Image.create(_module.Desc.create(_2_width, _3_height, (byte)(_4_channels), _module.ColorSpace.create_SRGB()), Dafny.Helpers.SeqFromArray(_0_input).Drop(new BigInteger(8)));
              byte[] _out2;
              BigInteger _out3;
              __default.encodeAll(_5_image, out _out2, out _out3);
              _6_result = _out2;
              _7_len = _out3;
              _8_repeat = (_8_repeat) + (BigInteger.One);
            }
    ...
    public static BigInteger encodePixelAEI(_IRGBA curr, _IRGBA prev, _IRGBA[] index, _IOp[] encoding, BigInteger len)
    {
      BigInteger newlen = BigInteger.Zero;
      newlen = (len) + (BigInteger.One);
      if (object.Equals(curr, prev)) {
        if ((((len).Sign == 1) && (((encoding)[(int)((len) - (BigInteger.One))]).is_OpRun)) && ((((encoding)[(int)((len) - (BigInteger.One))]).dtor_size) < ((byte)(62)))) {
          BigInteger _index0 = (len) - (BigInteger.One);
          (encoding)[(int)(_index0)] = _module.Op.create_OpRun((byte)((((encoding)[(int)((len) - (BigInteger.One))]).dtor_size) + ((byte)(1))));
          newlen = len;
        } else {
          (encoding)[(int)((len))] = _module.Op.create_OpRun((byte)(1));
        }
      } else if (object.Equals((index)[(int)(__default.hashRGBA(curr))], curr)) {
        (encoding)[(int)((len))] = _module.Op.create_OpIndex((byte)(__default.hashRGBA(curr)));
      } else if (!object.Equals(__default.canDiff(curr, prev), _module.Option<_IRGBDiff>.create_None())) {
        _IRGBDiff _0_result;
        _0_result = (__default.canDiff(curr, prev)).dtor_some;
        (encoding)[(int)((len))] = _module.Op.create_OpDiff(_0_result);
      } else if (!object.Equals(__default.canLuma(curr, prev), _module.Option<_IRGBLuma>.create_None())) {
        _IRGBLuma _1_result;
        _1_result = (__default.canLuma(curr, prev)).dtor_some;
        (encoding)[(int)((len))] = _module.Op.create_OpLuma(_1_result);
      } else if (((curr).dtor_a) == ((prev).dtor_a)) {
        (encoding)[(int)((len))] = _module.Op.create_OpRGB(_module.RGB.create((curr).dtor_r, (curr).dtor_g, (curr).dtor_b));
      } else {
        (encoding)[(int)((len))] = _module.Op.create_OpRGBA(_module.RGBA.create((curr).dtor_r, (curr).dtor_g, (curr).dtor_b, (curr).dtor_a));
      }
      byte _2_h;
      _2_h = __default.hashRGBA(curr);
      (index)[(int)((_2_h))] = curr;
      return newlen;
    }
    public static void encodeAEI(Dafny.ISequence<_IRGBA> image, out _IOp[] r, out BigInteger len)
    {
      r = new _IOp[0];
      len = BigInteger.Zero;
      _IOp[] _nw0 = Dafny.ArrayHelpers.InitNewArray1<_IOp>(Op.Default(), Dafny.Helpers.ToIntChecked(new BigInteger((image).Count), "array size exceeds memory limit"));
      r = _nw0;
      _IRGBA _0_prev;
      ...
      _IRGBA[] _nw1 = new _IRGBA[Dafny.Helpers.ToIntChecked(new BigInteger(64), "array size exceeds memory limit")];
      ...
      while ((_3_i) < (_4_wh)) {
        _IRGBA _5_curr;
        _5_curr = (image).Select(_3_i);
        BigInteger _out0;
        _out0 = __default.encodePixelAEI(_5_curr, _0_prev, _1_index, r, len);
        len = _out0;
        _0_prev = _5_curr;
        _3_i = (_3_i) + (BigInteger.One);
      }
    }
    public static void decodeOp(_IState state, _IOp op, BigInteger len, _IRGBA[] res, out BigInteger newlen, out _IState newstate, out bool ok)
    {
      newlen = BigInteger.Zero;
      ...
      _IOp _source0 = op;
      {
        if (_source0.is_OpRGB) {
          ...
          ok = ((len) + (BigInteger.One)) <= (new BigInteger((res).Length));
          if (ok) {
            _IRGBA _3_pixel;
            _3_pixel = _module.RGBA.create(_0_r, _1_g, _2_b, ((state).dtor_prev).dtor_a);
            (res)[(int)((len))] = _3_pixel;
            newlen = (len) + (BigInteger.One);
            newstate = __default.updateState(state, _3_pixel);
          }
          goto after_match0;
        }
      }
      {
        if (_source0.is_OpRun) {
          byte _4_size = _source0.dtor_size;
          {
            _IRGBA _5_pixel;
            _5_pixel = (state).dtor_prev;
            BigInteger _6_i;
            _6_i = BigInteger.Zero;
            newlen = len;
            newstate = state;
            Dafny.ISequence<_IRGBA> _7_pixels;
            _7_pixels = ((System.Func<Dafny.ISequence<_IRGBA>>) (() => {
              byte dim0 = _4_size;
              var arr0 = new _IRGBA[Dafny.Helpers.ToIntChecked(dim0, "array size exceeds memory limit")];
              for (int i0 = 0; i0 < dim0; i0++) {
                var _8_j = (byte) i0;
                arr0[(int)(_8_j)] = _5_pixel;
              }
              return Dafny.Sequence<_IRGBA>.FromArray(arr0);
            }))();
            ok = ((len) + (new BigInteger(_4_size))) <= (new BigInteger((res).Length));
            if (ok) {
              while ((_6_i) < (new BigInteger(_4_size))) {
                (res)[(int)((newlen))] = _5_pixel;
                newlen = (newlen) + (BigInteger.One);
                _6_i = (_6_i) + (BigInteger.One);
                newstate = __default.updateState(newstate, _5_pixel);
              }
            }
          }
          goto after_match0;
      ...
      }
      {
        if (_source0.is_OpIndex) {
          byte _9_index = _source0.dtor_index;
          ok = ((len) + (BigInteger.One)) <= (new BigInteger((res).Length));
          if (ok) {
            _IRGBA _10_pixel;
            _10_pixel = ((state).dtor_index).Select(_9_index);
            (res)[(int)((len))] = _10_pixel;
            newlen = (len) + (BigInteger.One);
            newstate = __default.updateState(state, _10_pixel);
          }
          goto after_match0;
        }
      }
      {
        if (_source0.is_OpDiff) {
          ...
          short _11_dr = diff0.dtor_dr;
          short _12_dg = diff0.dtor_dg;
          short _13_db = diff0.dtor_db;
          ok = ((len) + (BigInteger.One)) <= (new BigInteger((res).Length));
          if (ok) {
            _IRGBA _14_pixel;
            _14_pixel = _module.RGBA.create(__default.add__byte(((state).dtor_prev).dtor_r, __default.byte__from(new BigInteger(_11_dr))), __default.add__byte(((state).dtor_prev).dtor_g, __default.byte__from(new BigInteger(_12_dg))), __default.add__byte(((state).dtor_prev).dtor_b, __default.byte__from(new BigInteger(_13_db))), ((state).dtor_prev).dtor_a);
            (res)[(int)((len))] = _14_pixel;
            newlen = (len) + (BigInteger.One);
            newstate = __default.updateState(state, _14_pixel);
          }
          goto after_match0;
        }
      }
      {
        if (_source0.is_OpLuma) {
          ...
          short _15_dr = luma0.dtor_dr;
          short _16_dg = luma0.dtor_dg;
          short _17_db = luma0.dtor_db;
          ok = ((len) + (BigInteger.One)) <= (new BigInteger((res).Length));
          if (ok) {
            _IRGBA _18_pixel;
            _18_pixel = _module.RGBA.create(__default.add__byte(__default.add__byte(((state).dtor_prev).dtor_r, __default.byte__from(new BigInteger(_16_dg))), __default.byte__from(new BigInteger(_15_dr))), __default.add__byte(((state).dtor_prev).dtor_g, __default.byte__from(new BigInteger(_16_dg))), __default.add__byte(__default.add__byte(((state).dtor_prev).dtor_b, __default.byte__from(new BigInteger(_16_dg))), __default.byte__from(new BigInteger(_17_db))), ((state).dtor_prev).dtor_a);
            (res)[(int)((len))] = _18_pixel;
            newlen = (len) + (BigInteger.One);
            newstate = __default.updateState(state, _18_pixel);
          }
          goto after_match0;
        }
      }
      {
        _IRGBA _19_rgba = _source0.dtor_rgba;
        ok = ((len) + (BigInteger.One)) <= (new BigInteger((res).Length));
        if (ok) {
          _IRGBA _20_pixel;
          _20_pixel = _19_rgba;
          (res)[(int)((len))] = _20_pixel;
          newlen = (len) + (BigInteger.One);
          newstate = __default.updateState(state, _20_pixel);
        }
      }
    after_match0: ;
    }
    public static void decodeAEI(_IOp[] ops, BigInteger opsLen, BigInteger size, out _IRGBA[] r, out bool ok)
    {
      ...
      _0_i = BigInteger.Zero;
      _IState _1_state;
      _1_state = __default.initState();
      BigInteger _2_len;
      _2_len = BigInteger.Zero;
      ...
      while ((_0_i) < (opsLen)) {
        ...
        _IState _out1;
        bool _out2;
        __default.decodeOp(_1_state, _3_op, _2_len, r, out _out0, out _out1, out _out2);
        _2_len = _out0;
        _1_state = _out1;
        _4_ok_k = _out2;
        if (!(_4_ok_k)) {
    ...
    public static _IRGBA[] toRGBA3(Dafny.ISequence<byte> data)
    {
      _IRGBA[] result = new _IRGBA[0];
      _IRGBA[] _nw0 = Dafny.ArrayHelpers.InitNewArray1<_IRGBA>(RGBA.Default(), Dafny.Helpers.ToIntChecked(Dafny.Helpers.EuclideanDivision(new BigInteger((data).Count), new BigInteger(3)), "array size exceeds memory limit"));
      result = _nw0;
      BigInteger _0_i;
      _0_i = BigInteger.Zero;
      BigInteger _1_i3;
      _1_i3 = BigInteger.Zero;
      while ((_0_i) < (new BigInteger((data).Count))) {
        (result)[(int)((_1_i3))] = _module.RGBA.create((data).Select((_0_i) + (BigInteger.Zero)), (data).Select((_0_i) + (BigInteger.One)), (data).Select((_0_i) + (new BigInteger(2))), (byte)(255));
        _0_i = (_0_i) + (new BigInteger(3));
        _1_i3 = (_1_i3) + (BigInteger.One);
      }
      return result;
    }
    ...
    public static _IRGBA[] toRGBA4(Dafny.ISequence<byte> data)
    {
      _IRGBA[] result = new _IRGBA[0];
      _IRGBA[] _nw0 = Dafny.ArrayHelpers.InitNewArray1<_IRGBA>(RGBA.Default(), Dafny.Helpers.ToIntChecked(Dafny.Helpers.EuclideanDivision(new BigInteger((data).Count), new BigInteger(4)), "array size exceeds memory limit"));
      result = _nw0;
      BigInteger _0_i;
      _0_i = BigInteger.Zero;
      BigInteger _1_i4;
      _1_i4 = BigInteger.Zero;
      while ((_0_i) < (new BigInteger((data).Count))) {
        (result)[(int)((_1_i4))] = _module.RGBA.create((data).Select((_0_i) + (BigInteger.Zero)), (data).Select((_0_i) + (BigInteger.One)), (data).Select((_0_i) + (new BigInteger(2))), (data).Select((_0_i) + (new BigInteger(3))));
        _0_i = (_0_i) + (new BigInteger(4));
        _1_i4 = (_1_i4) + (BigInteger.One);
      }
      return result;
    }
    ...
    public static _IRGBA[] toRGBA(Dafny.ISequence<byte> data, _IDesc desc)
    {
      _IRGBA[] result = new _IRGBA[0];
      if (((desc).dtor_channels) == ((byte)(3))) {
        _IRGBA[] _out0;
        _out0 = __default.toRGBA3(data);
        result = _out0;
      } else {
        _IRGBA[] _out1;
        _out1 = __default.toRGBA4(data);
        result = _out1;
      }
      return result;
    }
    public static BigInteger writeHeader(_IDesc desc, byte[] result)
    {
      ...
      (result)[(int)((new BigInteger(7)))] = (__default.unpack((desc).dtor_width)).Select(new BigInteger(3));
      (result)[(int)((new BigInteger(8)))] = (__default.unpack((desc).dtor_height)).Select(BigInteger.Zero);
      (result)[(int)((new BigInteger(9)))] = (__default.unpack((desc).dtor_height)).Select(BigInteger.One);
      (result)[(int)((new BigInteger(10)))] = (__default.unpack((desc).dtor_height)).Select(new BigInteger(2));
      (result)[(int)((new BigInteger(11)))] = (__default.unpack((desc).dtor_height)).Select(new BigInteger(3));
      (result)[(int)((new BigInteger(12)))] = (byte)((desc).dtor_channels);
      (result)[(int)((new BigInteger(13)))] = __default.byteFromColorSpace((desc).dtor_colorSpace);
      len = new BigInteger(14);
      return len;
    }
    public static BigInteger writeFooter(byte[] result, BigInteger pos)
    {
      BigInteger newpos = BigInteger.Zero;
      BigInteger _index0 = (pos) + (BigInteger.Zero);
      (result)[(int)(_index0)] = (byte)(0);
      BigInteger _index1 = (pos) + (BigInteger.One);
      (result)[(int)(_index1)] = (byte)(0);
      BigInteger _index2 = (pos) + (new BigInteger(2));
      (result)[(int)(_index2)] = (byte)(0);
      BigInteger _index3 = (pos) + (new BigInteger(3));
      (result)[(int)(_index3)] = (byte)(0);
      BigInteger _index4 = (pos) + (new BigInteger(4));
      (result)[(int)(_index4)] = (byte)(0);
      BigInteger _index5 = (pos) + (new BigInteger(5));
      (result)[(int)(_index5)] = (byte)(0);
      BigInteger _index6 = (pos) + (new BigInteger(6));
      (result)[(int)(_index6)] = (byte)(0);
      BigInteger _index7 = (pos) + (new BigInteger(7));
      (result)[(int)(_index7)] = (byte)(1);
      newpos = (pos) + (new BigInteger(8));
      return newpos;
    }
    public static BigInteger writeBitsOp(_IOp op, byte[] result, BigInteger pos)
    {
      BigInteger newpos = BigInteger.Zero;
      newpos = (pos) + (BigInteger.One);
      _IOp _source0 = op;
      {
        if (_source0.is_OpRun) {
          byte _0_size = _source0.dtor_size;
          (result)[(int)((pos))] = (byte)(((byte)(((byte)(128)) + ((byte)(64)))) + ((byte)((new BigInteger(_0_size)) - (BigInteger.One))));
          goto after_match0;
        }
      }
      {
      ...
      }
      {
        if (_source0.is_OpDiff) {
          ...
          (result)[(int)((pos))] = (byte)(((byte)(((byte)(((byte)(64)) + ((byte)(((byte)(16)) * (__default.encodeDiff((_2_diff).dtor_dr)))))) + ((byte)(((byte)(4)) * (__default.encodeDiff((_2_diff).dtor_dg)))))) + (__default.encodeDiff((_2_diff).dtor_db)));
          goto after_match0;
        }
      }
      {
        if (_source0.is_OpLuma) {
          ...
          BigInteger _index0 = (pos) + (BigInteger.Zero);
          (result)[(int)(_index0)] = (byte)(((byte)(128)) + (__default.encodeDiff64((_3_luma).dtor_dg)));
          BigInteger _index1 = (pos) + (BigInteger.One);
          (result)[(int)(_index1)] = (byte)(((byte)(((byte)(16)) * (__default.encodeDiff16((_3_luma).dtor_dr)))) + (__default.encodeDiff16((_3_luma).dtor_db)));
          newpos = (pos) + (new BigInteger(2));
          goto after_match0;
        }
      }
      {
        if (_source0.is_OpRGB) {
          ...
          BigInteger _index2 = (pos) + (BigInteger.Zero);
          (result)[(int)(_index2)] = (byte)(254);
          BigInteger _index3 = (pos) + (BigInteger.One);
          (result)[(int)(_index3)] = (_4_rgb).dtor_r;
          BigInteger _index4 = (pos) + (new BigInteger(2));
          (result)[(int)(_index4)] = (_4_rgb).dtor_g;
          BigInteger _index5 = (pos) + (new BigInteger(3));
          (result)[(int)(_index5)] = (_4_rgb).dtor_b;
          newpos = (pos) + (new BigInteger(4));
          goto after_match0;
        }
      }
      {
        _IRGBA _5_rgba = _source0.dtor_rgba;
        BigInteger _index6 = (pos) + (BigInteger.Zero);
        (result)[(int)(_index6)] = (byte)(255);
        BigInteger _index7 = (pos) + (BigInteger.One);
        (result)[(int)(_index7)] = (_5_rgba).dtor_r;
        BigInteger _index8 = (pos) + (new BigInteger(2));
        (result)[(int)(_index8)] = (_5_rgba).dtor_g;
        BigInteger _index9 = (pos) + (new BigInteger(3));
        (result)[(int)(_index9)] = (_5_rgba).dtor_b;
        BigInteger _index10 = (pos) + (new BigInteger(4));
        (result)[(int)(_index10)] = (_5_rgba).dtor_a;
        newpos = (pos) + (new BigInteger(5));
      }
    after_match0: ;
      return newpos;
    }
    public static BigInteger encodeBitsOps(_IOp[] ops, BigInteger opsLen, byte[] result, BigInteger pos)
    {
      BigInteger newpos = BigInteger.Zero;
      BigInteger _0_i;
      _0_i = BigInteger.Zero;
      ...
      while ((_0_i) < (opsLen)) {
        BigInteger _out0;
        _out0 = __default.writeBitsOp((ops)[(int)(_0_i)], result, newpos);
        newpos = _out0;
        _0_i = (_0_i) + (BigInteger.One);
      }
      return newpos;
    }
    public static void encodeAll(_IImage image, out byte[] result, out BigInteger len)
    {
      result = new byte[0];
      len = BigInteger.Zero;
      _IOp[] _0_ops = new _IOp[0];
      BigInteger _1_opsLen = BigInteger.Zero;
      _IRGBA[] _2_arrayRGBA;
      _IRGBA[] _out0;
      _out0 = __default.toRGBA((image).dtor_data, (image).dtor_desc);
      _2_arrayRGBA = _out0;
      _IOp[] _out1;
      BigInteger _out2;
      __default.encodeAEI(Dafny.Helpers.SeqFromArray(_2_arrayRGBA), out _out1, out _out2);
      _0_ops = _out1;
      _1_opsLen = _out2;
      byte[] _nw0 = new byte[Dafny.Helpers.ToIntChecked((((_1_opsLen) * (new BigInteger(5))) + (new BigInteger(14))) + (new BigInteger(8)), "array size exceeds memory limit")];
      result = _nw0;
      BigInteger _out3;
      _out3 = __default.writeHeader((image).dtor_desc, result);
      len = _out3;
      BigInteger _out4;
      _out4 = __default.encodeBitsOps(_0_ops, _1_opsLen, result, len);
      len = _out4;
      BigInteger _out5;
      _out5 = __default.writeFooter(result, len);
      len = _out5;
    }
    public static _IOption<_IDesc> parseHeader(Dafny.ISequence<byte> byteStream)
    {
      ...
      if (((((byte)(3)) <= ((byteStream).Select(new BigInteger(12)))) && (((byteStream).Select(new BigInteger(12))) <= ((byte)(4)))) && (__default.validColorSpaceAsByte((byteStream).Select(new BigInteger(13))))) {
        _IDesc _0_desc;
        _0_desc = _module.Desc.create(__default.pack((byteStream).Subsequence(new BigInteger(4), new BigInteger(8))), __default.pack((byteStream).Subsequence(new BigInteger(8), new BigInteger(12))), (byte)((byteStream).Select(new BigInteger(12))), __default.colorSpaceFromByte((byteStream).Select(new BigInteger(13))));
    ...
    public static _IOption<_System._ITuple2<_IDesc, byte[]>> decodeAll(byte[] byteStream)
    {
      ...
      if ((new BigInteger((byteStream).Length)) < ((new BigInteger(14)) + (new BigInteger(8)))) {
        ...
        BigInteger _0_len;
        _0_len = new BigInteger((byteStream).Length);
        Dafny.ISequence<byte> _1_header;
        _1_header = Dafny.Helpers.SeqFromArray(byteStream).Take(new BigInteger(14));
        ...
        _IOption<_IOp[]> _5_opsOption = Option<_IOp[]>.Default();
        BigInteger _6_len_k = BigInteger.Zero;
        _IOption<_IOp[]> _out1;
        BigInteger _out2;
        __default.decodeBitsOps(byteStream, new BigInteger(14), (_0_len) - (new BigInteger(8)), out _out1, out _out2);
        _5_opsOption = _out1;
        _6_len_k = _out2;
        ...
        _IOp[] _7_ops;
        _7_ops = (_5_opsOption).dtor_some;
        _IRGBA[] _8_rgbs = new _IRGBA[0];
        bool _9_ok = false;
        _IRGBA[] _out3;
        bool _out4;
    ...
    public static Dafny.ISequence<byte> encodeOpSpec(_IOp op) {
      _IOp _source0 = op;
      {
        if (_source0.is_OpRun) {
          byte _0_size = _source0.dtor_size;
    ...
    public static bool areValidBits(byte[] bits, BigInteger start, BigInteger len)
    {
      bool b = false;
      b = ((len).Sign == 1) && (((System.Func<bool>)(() => {
        _IOpType _source0 = __default.opTypeOfBits((bits)[(int)((start) + (BigInteger.Zero))]);
        {
          if (_source0.is_TypeRun) {
            return (((len) == (BigInteger.One)) && (((bits)[(int)((start) + (BigInteger.Zero))]) >= ((byte)(((byte)(128)) + ((byte)(64)))))) && (((byte)(((byte)(((bits)[(int)((start) + (BigInteger.Zero))]) - ((byte)(128)))) - ((byte)(64)))) <= ((byte)(61)));
          }
        }
        {
          if (_source0.is_TypeLuma) {
            return (((len) == (new BigInteger(2))) && (((bits)[(int)((start) + (BigInteger.Zero))]) >= ((byte)(128)))) && (((bits)[(int)((start) + (BigInteger.Zero))]) < ((byte)(((byte)(128)) + ((byte)(64)))));
          }
        }
        {
          if (_source0.is_TypeDiff) {
            return (((len) == (BigInteger.One)) && (((bits)[(int)((start) + (BigInteger.Zero))]) >= ((byte)(64)))) && (((bits)[(int)((start) + (BigInteger.Zero))]) < ((byte)(128)));
          }
        }
        {
          if (_source0.is_TypeIndex) {
            return ((len) == (BigInteger.One)) && (((bits)[(int)((start) + (BigInteger.Zero))]) < ((byte)(64)));
          }
        }
        {
          if (_source0.is_TypeRGBA) {
            return ((len) == (new BigInteger(5))) && (((bits)[(int)((start) + (BigInteger.Zero))]) == ((byte)(255)));
          }
        }
        {
          return ((len) == (new BigInteger(4))) && (((bits)[(int)((start) + (BigInteger.Zero))]) == ((byte)(254)));
        }
      }))());
      return b;
    }
    ...
    public static Dafny.ISequence<byte> encodeOpsSpec(Dafny.ISequence<_IOp> ops) {
    ...
    TAIL_CALL_START: ;
      if ((new BigInteger((ops).Count)).Sign == 0) {
        ...
        _0___accumulator = Dafny.Sequence<byte>.Concat(_0___accumulator, __default.encodeOpSpec(__default.head<_IOp>(ops)));
        Dafny.ISequence<_IOp> _in0 = __default.tail<_IOp>(ops);
        ops = _in0;
        goto TAIL_CALL_START;
      }
    }
    public static __T head<__T>(Dafny.ISequence<__T> l) {
      return (l).Select(BigInteger.Zero);
    }
    public static Dafny.ISequence<__T> tail<__T>(Dafny.ISequence<__T> l) {
      return (l).Drop(BigInteger.One);
    }
    public static Dafny.ISequence<__T> init<__T>(Dafny.ISequence<__T> l) {
      return (l).Take((new BigInteger((l).Count)) - (BigInteger.One));
    }
    public static __T last<__T>(Dafny.ISequence<__T> l) {
      return (l).Select((new BigInteger((l).Count)) - (BigInteger.One));
    }
    public static Dafny.ISequence<_IOp> specBits(Dafny.ISequence<byte> bits) {
    ...
    public static void decodeBitsOps(byte[] bits, BigInteger start, BigInteger end, out _IOption<_IOp[]> r, out BigInteger lenr)
    {
      r = Option<_IOp[]>.Default();
      lenr = BigInteger.Zero;
      BigInteger _0_i;
      _0_i = BigInteger.Zero;
      BigInteger _1_j;
      _1_j = BigInteger.Zero;
      _IOp[] _2_result;
      _IOp[] _nw0 = Dafny.ArrayHelpers.InitNewArray1<_IOp>(Op.Default(), Dafny.Helpers.ToIntChecked((end) - (start), "array size exceeds memory limit"));
      _2_result = _nw0;
      while ((_0_i) < ((end) - (start))) {
        BigInteger _3_len;
        _3_len = __default.sizeBitEncoding(__default.opTypeOfBits((bits)[(int)((start) + (_0_i))]));
        if (((_0_i) + (_3_len)) > ((end) - (start))) {
          _IOption<_IOp[]> _rhs0 = _module.Option<_IOp[]>.create_None();
          BigInteger _rhs1 = _1_j;
          r = _rhs0;
          lenr = _rhs1;
          return ;
        } else {
          bool _4_b;
          bool _out0;
          _out0 = __default.areValidBits(bits, (start) + (_0_i), _3_len);
          _4_b = _out0;
          if (_4_b) {
            (_2_result)[(int)((_1_j))] = __default.decodeBits(Dafny.Helpers.SeqFromArray(bits).Subsequence((start) + (_0_i), ((start) + (_0_i)) + (_3_len)));
            _1_j = (_1_j) + (BigInteger.One);
            _0_i = (_0_i) + (_3_len);
          } else {
            _IOption<_IOp[]> _rhs2 = _module.Option<_IOp[]>.create_None();
            BigInteger _rhs3 = _1_j;
            r = _rhs2;
            lenr = _rhs3;
            return ;
          }
      ...
      _IOption<_IOp[]> _rhs4 = _module.Option<_IOp[]>.create_Some(_2_result);
      BigInteger _rhs5 = _1_j;
      r = _rhs4;
      lenr = _rhs5;
      return ;
    }
    ...
    public static Dafny.ISequence<byte> specFooter() {
    ...
    public static bool validFooter(Dafny.ISequence<byte> bits) {
      return (bits).Equals(__default.specFooter());
    }
    public static bool validByteStream(Dafny.ISequence<byte> byteStream) {
      BigInteger _0_len = new BigInteger((byteStream).Count);
      return (((((new BigInteger((byteStream).Count)) >= ((new BigInteger(14)) + (new BigInteger(8)))) && (__default.validHeader((byteStream).Subsequence(BigInteger.Zero, new BigInteger(14))))) && (__default.validFooter((byteStream).Drop((_0_len) - (new BigInteger(8)))))) && (__default.validBitSeq((byteStream).Subsequence(new BigInteger(14), (_0_len) - (new BigInteger(8)))))) && ((new BigInteger((__default.specOps(__default.specBits((byteStream).Subsequence(new BigInteger(14), (_0_len) - (new BigInteger(8)))))).Count)) == ((new BigInteger((__default.specHeaderDesc((byteStream).Subsequence(BigInteger.Zero, new BigInteger(14)))).dtor_width)) * (new BigInteger((__default.specHeaderDesc((byteStream).Subsequence(BigInteger.Zero, new BigInteger(14)))).dtor_height))));
    }
    public static _IImage specEndToEnd(Dafny.ISequence<byte> byteStream) {
      _IDesc _0_desc = __default.specHeaderDesc((byteStream).Take(new BigInteger(14)));
      BigInteger _1_len = new BigInteger((byteStream).Count);
      return _module.Image.create(_0_desc, __default.toByteStream(_0_desc, __default.specOps(__default.specBits((byteStream).Subsequence(new BigInteger(14), (_1_len) - (new BigInteger(8)))))));
    }
